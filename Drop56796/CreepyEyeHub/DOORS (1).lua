--[[
    DOORS (1).lua  —  organised by minimarchbleeding
    Original : Drop56796/CreepyEyeHub  ->  DOORS (1).txt
    Clean loader (not obfuscated); pulls: https://raw.githubusercontent.com/RegularVynixu/Utilities/main/Discord%20Inviter/Source.lua
--]]
...
local Window = Library:CreateWindow({
	Title = "Mspaint v2 (谁他妈要是汉化我操死谁的妈)",
	Center = true,
	AutoShow = true,
	Resizable = true,
...
local Tabs = {
    ...
    Floor = Window:AddTab("二层"),
	["UI Settings"] = Window:AddTab("UI栏没必要看"),
}
...
function Script.Functions.DoorESP(room)
    ...
    if door then
        ...
        door:GetAttributeChangedSignal("Opened"):Connect(function()
            doorEsp.Text = string.format("门 %s [Opened]", doorNumber)
        end)
    end
end 
...
function Script.Functions.ObjectiveESP(child)
    ...
    if child.Name == "TimerLever" then
        Script.Functions.ESP({
            Type = "Objective",
            Object = child,
            Text = string.format("拉杆 [+%s]", child.TakeTimer.TextLabel.Text),
...
function Script.Functions.ChestESP(chest)
    ...
    Script.Functions.ESP({
        Type = "Chest",
        Object = chest,
        Text = locked and "箱子 [Locked]" or "箱子",
...
local MiscGroupBox = Tabs.Main:AddRightGroupbox("其他") do
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("删除") do
    ...
    AntiEntityGroupBox:AddToggle("AntiDupe", {
        Text = "移除" .. (isBackdoor and "Vacuum" or "和蔼的假门"),
        Default = false
    })

    AntiEntityGroupBox:AddToggle("AntiEyes", {
        Text = "移除老六" .. (isBackdoor and "Lookman" or "眼睛"),
        Default = false
    })
...
local BypassGroupBox = Tabs.Exploits:AddRightGroupbox("绕过功能") do
    BypassGroupBox:AddDropdown("我也不知道怎么用啊", {
        AllowNull = false,
...
local AmbientGroupBox = Tabs.Visuals:AddLeftGroupbox("其余功能") do
